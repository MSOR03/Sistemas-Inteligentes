# -*- coding: utf-8 -*-
"""
RIVAL DE PRUEBA - Minimax de Knuth  (interfaz `compute`)

Elige la jugada que minimiza el GRUPO DE RESPUESTA MAS GRANDE, es decir la que
menos candidatos deja en el peor caso. Desempata prefiriendo una jugada que
todavia pueda ser el secreto, y luego por orden.

Es la estrategia clasica de Mastermind. Baja el peor caso, pero no el
promedio: medido sobre los 5040 secretos da 5.34 turnos frente a los 5.24 de
puntuar por turnos esperados.
...
class RivalMinimax(_Base):
    """Primer intento 0123 y despues minimax sobre las 5040 jugadas."""
    ...
    def _decidir(self):
        ...
        if len(vivos) <= 2:                       # jugar un candidato ya gana
            return vivos[0] if vivos else CODIGOS[0]
        mejor, mejor_clave = None, None
        for i, codigo in enumerate(CODIGOS):
            peor = max(conteos(codigo, conjunto))
            clave = (peor, 0 if (conjunto >> i) & 1 else 1)
            if mejor_clave is None or clave < mejor_clave:
                mejor_clave, mejor = clave, codigo
        return mejor
